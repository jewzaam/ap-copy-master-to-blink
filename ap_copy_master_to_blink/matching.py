...
- DATE from path (DATE_2026-02-07) still overrides file headers (file_naming_override=True)
...
def find_matching_flat(
    ...
    if matches:
        if len(matches) > 1:
            logger.warning(
                f"Found {len(matches)} matching flats for date {light_metadata.get(NORMALIZED_HEADER_DATE)} "
